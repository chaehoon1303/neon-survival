...
let operativeRoster=JSON.parse(localStorage.neonOperativeRoster||'{}'),operativeRanks=JSON.parse(localStorage.neonOperativeRanks||'{}'),operativeAcquiredOrder=JSON.parse(localStorage.neonOperativeAcquiredOrder||'{}'),operativeClears=Number(localStorage.neonOperativeClears||0),operativeSort=localStorage.neonOperativeSort||'tier-desc';
...
if(!CHARACTER_DATA[selectedCharacter]||!operativeRoster[selectedCharacter])selectedCharacter='recruit';
function saveOperatives(){localStorage.neonOperativeRoster=JSON.stringify(operativeRoster);localStorage.neonOperativeRanks=JSON.stringify(operativeRanks);localStorage.neonOperativeAcquiredOrder=JSON.stringify(operativeAcquiredOrder);localStorage.neonOperativeClears=String(operativeClears);localStorage.neonOperativeSort=operativeSort;localStorage.neonOperativeCrateKeys=String(operativeCrateKeys);localStorage.neonSelectedCharacter=selectedCharacter}
...
function renderCharacterList(){
  ...
  list.innerHTML=orderedOperatives().map(([id,data])=>{
    const copies=operativeRoster[id]||0,rank=operativeRank(id),owned=copies>0,canUpgrade=copies>=3&&rank<5,icon=id==='kairos'?'⚔':id==='lumina'?'✦':id==='volt'?'ϟ':id==='gravion'?'◉':'❄';
    return `<article class="character-card ${id===selectedCharacter?'selected':''} ${owned?'':'locked'} ${data.tier}" style="--char:${data.color}"><span>${owned?icon:'🔒'}</span><b>${data.name}</b><small>${OPERATIVE_TIERS[data.tier]} · ${data.role}</small><em>HP ${data.hp} · 공격 ${data.damage}<br>강화 ${rank}/5 · 보유 ${copies}명</em><i>Q ${data.q.name}<br>E ${data.e.name}<br>R ${data.r.name}</i><button data-character="${id}" ${owned?'':'disabled'}>${owned?'선택':'미획득'}</button><button data-operative-upgrade="${id}" ${canUpgrade?'':'disabled'}>${canUpgrade?'동일 요원 3명으로 강화':'강화에 동일 요원 3명 필요'}</button></article>`;
  }).join('');
  list.querySelectorAll('[data-character]').forEach(button=>button.onclick=()=>{selectedCharacter=button.dataset.character;agentSpriteId='';refreshAgentTexture();saveOperatives();renderCharacterList();pop(`${characterNow().name} 선택 완료`)});
  list.querySelectorAll('[data-operative-upgrade]').forEach(button=>button.onclick=()=>{const id=button.dataset.operativeUpgrade;if((operativeRoster[id]||0)<3)return;operativeRoster[id]-=3;operativeRanks[id]=Math.min(5,operativeRank(id)+1);saveOperatives();renderCharacterList();pop(`${CHARACTER_DATA[id].name} 강화 ${operativeRank(id)}단계! HP·공격 +10%, 스킬 피해 +12%, 쿨타임 -5%`)});
}
...
renderCharacterList=function(){coreTransRenderCharacterList();const list=$('#character-list');if(!list)return;list.querySelectorAll('.character-card.transcendence').forEach(card=>{const details=card.querySelector('i');if(details)details.innerHTML+=`<br>F ${CHARACTER_DATA[card.querySelector('[data-character]')?.dataset.character||Object.keys(CHARACTER_DATA).find(id=>CHARACTER_DATA[id].name===card.querySelector('b')?.textContent)]?.f?.name||'초월기'}`});};
...
const storedAnimeSelection=localStorage.neonSelectedCharacter;if(ANIME_EVENT_CHARACTER_CONFIG[storedAnimeSelection]&&operativeRoster[storedAnimeSelection])selectedCharacter=storedAnimeSelection;
...
function awardOperative(id,source){operativeRoster[id]=(operativeRoster[id]||0)+1;operativeAcquiredOrder[id]=Date.now();saveOperatives();return`${source}: ${OPERATIVE_TIERS[CHARACTER_DATA[id].tier]} 요원 ${CHARACTER_DATA[id].name} 획득!`}
...
const storedLowerSelection=localStorage.neonSelectedCharacter;if(isLowerRarityCharacter(storedLowerSelection)&&operativeRoster[storedLowerSelection])selectedCharacter=storedLowerSelection;
...
const storedBladeSelection=localStorage.neonSelectedCharacter;if(isBladeEventCharacter(storedBladeSelection)&&(operativeRoster[storedBladeSelection]||bladeEventTestMode()))selectedCharacter=storedBladeSelection;
