...
setTimeout(()=>{
  ...
  itemIcon=item=>item.slot==='weapon'?(weaponIcons[item.name]||simpleItemGlyph(item)):simpleItemGlyph(item);
  drawGear=()=>{
...
function mapButtons(){let m=maps[selected],locked=selected>unlocked;ui.picker.innerHTML=`<button class="map selected ${locked?'locked':''}"><b>${locked?'<i class="game-icon icon-lock" aria-label="잠김"></i> ':''}${m.name}</b><small>${locked?'이전 맵 클리어 필요':`${5+selected*2} 스테이지 · 보스전`}</small></button>`;$('#map-prev').disabled=selected===0;$('#map-next').disabled=selected===maps.length-1}
...
function simpleItemGlyph(item){const icon=item?.slot==='weapon'?'icon-sword':({armor:'icon-armor',gloves:'icon-gloves',shoes:'icon-shoes',belt:'icon-belt',necklace:'icon-necklace',head:'icon-head'})[item?.slot]||'icon-generic';return `<span class="simple-item-glyph" aria-hidden="true"><i class="game-icon ${icon}"></i></span>`}function itemIcon(item){return simpleItemGlyph(item)}function buy(kind){if(wallet<100){$('#shop-result').textContent='코인이 부족합니다.';return}wallet-=100;let item=rollItem(kind==='weapon'?weapons:armors);inventory.push(item);saveGear();renderCoins();$('#shop-result').innerHTML=`<div class="loot-texture ${item.tier}"><span class="loot-icon">${itemIcon(item)}</span><b>${item.name}</b><small>${item.desc}</small><small>${({common:'일반',rare:'희귀',hero:'영웅',legend:'전설',mythic:'신화'})[item.tier]}</small></div>`;drawGear()}function showShop(){$('#menu').classList.add('hidden');$('#shop').classList.remove('hidden');$('#shop-result').textContent='상자를 열어 보상을 확인하세요.'}$('#crate-button').onclick=showShop;document.querySelectorAll('.close').forEach(b=>b.onclick=()=>{$('#'+b.dataset.close).classList.add('hidden');$('#menu').classList.remove('hidden')});
