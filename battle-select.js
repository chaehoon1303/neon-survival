...
(function () {
  ...
  function battleRender() {
    ...
    document.querySelector('#battle-stage-name').innerHTML = (locked ? '<i class="game-icon icon-lock" aria-label="잠김"></i> ' : '') + map.name;
    ...
    document.querySelector('#battle-reward-coins').innerHTML = '<i class="game-icon icon-coin" aria-hidden="true"></i> +' + reward; document.querySelector('#battle-reward-extra').innerHTML = '<i>◈</i><i>✦</i><i>▣</i>'; document.querySelector('#battle-reward-note').textContent = locked ? '이전 지역 정복 필요' : selected === unlocked && selected < maps.length - 1 ? '클리어 시 다음 지역 해금' : '정복 보상 + 요원 신호';
